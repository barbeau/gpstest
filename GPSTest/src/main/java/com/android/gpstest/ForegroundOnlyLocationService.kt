...
class ForegroundOnlyLocationService : LifecycleService() {
    ...
    private var currentStatus: List<SatelliteStatus> = emptyList()
        ...
        locationFlow = repository.getLocations()
            ...
            .onEach {
                ...
                notificationManager.notify(
                    NOTIFICATION_ID,
                    buildNotification(it, currentStatus)
                )
    ...
    private fun observeGnssFlow() {
        ...
        gnssFlow = repository.getGnssStatus()
            ...
            .onEach {
                ...
                notificationManager.notify(
                    NOTIFICATION_ID,
                    buildNotification(currentLocation, currentStatus)
                )
            }
            .launchIn(lifecycleScope)
    }
    ...
    private fun postOngoingActivityNotification() {
        if (!isForeground) {
            ...
            startForeground(NOTIFICATION_ID, buildNotification(currentLocation, currentStatus))
        }
    }
     ...
     */
    private fun buildNotification(location: Location?, status: List<SatelliteStatus>): Notification {
        val titleText = status.toSatelliteGroup().toNotificationTitle()
